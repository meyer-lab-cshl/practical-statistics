...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#to have a table of proportions based on rows}
...
\StringTok{  }\KeywordTok{as.data.frame.matrix}\NormalTok{() }\CommentTok{#convert it to a dataframe}
\CommentTok{#have a look at the table}
...
\begin{verbatim}
##      A    B    C    D    E
## a 0.25 0.25 0.25 0.00 0.25
## b 0.40 0.00 0.20 0.40 0.00
## c 0.00 0.50 0.25 0.25 0.00
## d 0.00 0.40 0.20 0.00 0.40
## e 0.00 0.50 0.00 0.50 0.00
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#to have a table of proportions based on columns}
...
\StringTok{  }\KeywordTok{as.data.frame.matrix}\NormalTok{() }\CommentTok{#convert it to dataframe}
\CommentTok{#have a look at the table}
\KeywordTok{print.data.frame}\NormalTok{(my_table_}\DecValTok{2}\NormalTok{, }\DataTypeTok{digits =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
For the AIP test: \#\# Sensitivity: (AIP example) the probability of
testing positive given that the subjevt has the disease

\(Sensitivity = p(test^+|disease^+)=82%\)

\hypertarget{specificity-aip-example-the-probability-of-a-negative-test-given-that-the-subject-does-not-have-the-disease}{%
\subsection{Specificity: (AIP example) the probability of a negative
test given that the subject does not have the
disease}\label{specificity-aip-example-the-probability-of-a-negative-test-given-that-the-subject-does-not-have-the-disease}}

\(Specificity = p(test^-|disease^-)=96.3%\)

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# youtube vid: https://www.youtube.com/watch?v=9f5XgjWpzi0}
...
\hypertarget{positive-predictive-value-ppv-the-probability-that-subjects-with-a-positive-screening-test-truly-have-the-disease}{%
\subsection{Positive Predictive Value (PPV): the probability that
subjects with a positive screening test truly have the
disease}\label{positive-predictive-value-ppv-the-probability-that-subjects-with-a-positive-screening-test-truly-have-the-disease}}
...
disease. \(prevalence = p(disease^+) = 50%\)

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Usage: prevalence(model, type = c("pop", "bnp", "wnp"), i=NULL, ...)}
...
\CommentTok{# First 10 observations.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#prevalence can be calculated as the number of positive diagnoses divided by the total number of observations, i.e. sum(Lyme)/n()}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Syntax: chisp.test(data)}
...
\CommentTok{##calculate standard error manually}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#intall psych package in console}
\CommentTok{#Use describe function from psych package for standard error}
\CommentTok{#This function also works on whole dataframes}
...
\hypertarget{z-tests}{%
\subsection{Z-Tests}\label{z-tests}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# independent 2-sample t-test, where y is numeric and x is a binary factor}
...
\KeywordTok{t.test}\NormalTok{(y,}\DataTypeTok{mu=}\DecValTok{3}\NormalTok{) }\CommentTok{# Ho(null hypothesis): mu=3}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#in console: install.packages("car")}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-15-1.pdf}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-15-2.pdf}
...
with an r-squared of 0.81146, then approximately 81.15\% of the observed
...
*Example: The function tells us we should flipthe coin 22.55126 times,
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-17-1.pdf}
...
\subsection{ANOVA tests (Analysis of
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Compute the mean and the standard deviation}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Graphically check if there is a difference between the distribution. }
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-19-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Run the one-way ANOVA test with the command aov. }
\CommentTok{#basic syntax for ANOVA test: aov(formula, data)}
...
\CommentTok{#First, visualize the scatter plot of the medv vs lstat variables:}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-20-1.pdf}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-21-1.pdf}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-22-1.pdf}
...
\includegraphics{PracticalStatisticsR_files/figure-latex/unnamed-chunk-23-1.pdf}
